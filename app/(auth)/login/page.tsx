...
function LoginForm() {
  ...
  async function handleGoogleLogin() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/api/auth/confirm?redirect=${redirect}`,
      },
    })
        ...
        <CardContent>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="bg-[#2a3942] px-2 text-[#71828A]">o continúa con</span>
            </div>
          </div>
          ...
          <p className="mt-6 text-center text-sm text-[#71828A]">
            ¿No tienes cuenta?{' '}
            <Link href="/register" className="text-[#00D47F] hover:underline">
              Crear cuenta
            </Link>
          </p>
        </CardContent>
...
export default function LoginPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#233037] flex items-center justify-center">
